...
has _decodedConfig => (
  ...
  default => sub {
    my $self = shift;
    return LoadFile( $self->configPath );
  }
);
...
has _newConfigPath => (
  ...
  default => sub {
    ...
    return
        substr( $self->configPath, 0, rindex( $self->config, '.' ) ) . "."
      . $self->_dateOfRun
...
sub BUILD {
  ...
  my $trackIndex = first_index { $_->{name} eq $self->name }
  @{ $self->_decodedConfig->{tracks}{tracks} };
...
sub _backupAndWriteConfig {
  ...
  open( my $fh, '>', $self->_newConfigPath )
    or $self->log( 'fatal', "Couldn't open" . $self->_newConfigPath . " for writing" );
  ...
  if ( system( "ln -f " . $self->_newConfigPath . " " . $self->configPath ) != 0 ) {
    $self->log( 'fatal',
      "Failed to hard link " . $self->configPath . " to " . $self->_newConfigPath );
  }
