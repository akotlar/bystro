...
sub go {
  ...
  for my $inPath (@$localFilesPathsAref) {
    ...
    if ( !$isCompressed ) {
      $command =
        "$liftOverExe <(cat $inPath | tail -n +3) $chainPath /dev/stdout $unmappedPath -bedPlus=3 ";
      if ($compressOutput) {
      ...
      $command =
        "$liftOverExe <($gzip -d -c $inPath | tail -n +3) $chainPath /dev/stdout $unmappedPath -bedPlus=3 | $gzip -c - >> $liftedPath; $gzip $unmappedPath";
    }
    ...
    if ( $exitStatus != 0 ) {
      $self->log( 'fatal',
        "liftOver command for $inPath failed with exit status: $exitStatus" );
    }
    else {
      $self->log( 'info',
        "Successfully completed liftOver with with exit status: $exitStatus" );
    }
