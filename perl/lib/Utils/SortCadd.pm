...
sub go {
    ...
    while ( my $l = $readFh->getline() ) {
      ...
      if ( !$fh ) {
        ...
        if ( -e $outPath && !$self->overwrite ) {
          $self->log( 'warn',
            "outPath $outPath exists, skipping $inFilePath because overwrite not set" );
          last;
        }
  ...
  $self->log( 'info',
    "Finished organizing cadd files by chr, beginning sort (multi threaded)" );
  ...
  $pm->run_on_finish(
    sub {
      ...
      if ( $exitCode != 0 ) {
        return $self->log( 'fatal',
          "$finalOutPath failed to sort, with exit code $exitCode" );
      }
  ...
  for my $outPath (@outPaths) {
    ...
    if ($compressed) {
      $command =
        "( head -n 2 <($gzipPath -d -c $outPath) && tail -n +3 <($gzipPath -d -c $outPath) | sort --compress-program $gzipPath -T $tempPath -k2,2 -n ) | $gzipPath -c > $finalOutPath";
    }
    else {
      $command =
        "( head -n 2 $outPath && tail -n +3 $outPath | sort --compress-program $gzipPath -T $tempPath -k2,2 -n ) > $finalOutPath";
    }
    ...
    if ( $exitStatus == 0 ) {
      $self->log( 'info',
        "Successfully finished sorting $outPath. Exit status: $exitStatus" );
