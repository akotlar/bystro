...
async def async_stream_file(
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(
            url, headers=auth_header, json=payload, timeout=MAX_STREAM_TIMEOUT
        ) as response:
            if response.status == 200:
                ...
                if chunk_size is not None:
                    response_iterator = response.content.iter_chunked(chunk_size)
                else:
                    response_iterator = response.content.iter_any()
            ...
            if response.status == 400:
                raise RuntimeError(f"Bad Request: {await response.text()}")
            
            if response.status == 404:
                raise RuntimeError("File not found.")
            
            raise RuntimeError(f"Failed to fetch file. Error code: {response.status}")

async def async_stream_and_decompress_file(
    job_id: str,
    output: bool = True,
    key_path: Optional[str] = "annotation",
    ...
    stream = await async_stream_file(job_id, output=output, key_path=key_path)
